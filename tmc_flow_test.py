...
#
# Uses TMC StallGuard (and optionally CoolStep CS_ACTUAL) load monitoring
# to find the maximum volumetric flow rate where the extruder still moves
# filament reliably. No visual inspection or test prints needed.
#
# Supported drivers:
#   - TMC2240 (uses SG4_RESULT register, sg4_thrs/sg4_filt_en fields)
#   - TMC2209 (uses SG_RESULT register, sgthrs field, no filter)
#   - TMC5160 / TMC2130 / TMC2208 / TMC2226 (basic SG2 fallback)
#
# IMPORTANT: run the test with the same TMC settings you use during
# normal printing. The plugin auto-detects the right test mode from
# your driver_SEMIN setting.
#
# Main command:
#
#   TMC_FLOW_FIND_MAX
#     Auto-detects test mode from driver_SEMIN:
#       driver_SEMIN: 0  → SG-only mode
#       driver_SEMIN > 0 → CoolStep + SG mode
#     Optional override: MODE=sg, MODE=cs, MODE=auto (default)
#
# Explicit-mode commands (still available):
#
#   TMC_FLOW_FIND_MAX_SG
#     Force SG-only detection. Equivalent to TMC_FLOW_FIND_MAX MODE=sg.
#     Detection: SG abnormal jump + plateau over 2 steps.
#
#   TMC_FLOW_FIND_MAX_CS
#     Force CoolStep + StallGuard detection.
#     Equivalent to TMC_FLOW_FIND_MAX MODE=cs.
#     Detection: 4 triggers (CS jump, CS leaves regulation,
#     CS hard-drop, SG abnormal jump as backup).
#
# Algorithm (both modes): bracket-bisection
#   Phase 1 (Coarse): increase flow in big steps until trigger fires
#   Phase 2 (Bisection): narrow bracket by halving until ≤ MIN_STEP
#   Phase 3 (Verification): confirm safe value with extra repeats
#
# Hardware setup (printer.cfg):
#
#   For driver_SEMIN: 0  →  use TMC_FLOW_FIND_MAX_SG
#     [tmc2240 extruder]   (or [tmc2209 extruder])
#         stealthchop_threshold: 999999
#         coolstep_threshold: 0.5
#         driver_SEMIN: 0
#
#   For driver_SEMIN > 0  →  use TMC_FLOW_FIND_MAX_CS
#     [tmc2240 extruder]   (or [tmc2209 extruder])
#         stealthchop_threshold: 999999
#         coolstep_threshold: 0.5
#         driver_SEMIN: 5
...
#         driver_SEIMIN: 1
#
#   StallGuard threshold setup (TMC2240):
...
#
#   StallGuard threshold setup (TMC2209):
#     [delayed_gcode setup_extruder_sg]
#         initial_duration: 2.0
#         gcode:
