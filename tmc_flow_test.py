...
%(cs_chart_script)s
    ...
    def _check_sg_max_spike(self, results, sg_label):
        ...
        the repeats, SG snaps toward its no-load value (much HIGHER on
        SG2 drivers, much LOWER on SG4 — direction depends on driver
        family). The 5-second per-run average + median over 5 runs
        absorbs the brief spike, but sg_max (or sg_min) records it.

        We compare last_sg_max to:
          - the median across previous bisect/coarse steps' sg_max
          - the current step's sg_median
        If last_sg_max is dramatically larger than both, it's a
        decoupling event in at least one of the repeats.

        Only fires in bisection / verify, since coarse already has
        plenty of other triggers and natural variation is wider there.
        """
        last = results[-1]
        ...
        # Build baseline of "typical" sg_max from the coarse phase.
        coarse_maxes = []
        for r in results[:-1]:
            if r.get('phase', 'coarse') != 'coarse':
                continue
        ...
        if len(coarse_maxes) < 4:
            return None

        # Drop last coarse step (likely elevated). Use median of rest.
        sorted_max = sorted(coarse_maxes[:-1])
        n = len(sorted_max)
        median_coarse_max = (sorted_max[n // 2] if n % 2
                             else (sorted_max[n // 2 - 1]
                                   + sorted_max[n // 2]) / 2)

        # SG2 drivers (TMC5160 etc): SG INCREASES on decoupling
        # (snap toward no-load). SG4 drivers: SG DECREASES.
        # We test both directions independently.
        if self.sg2_driver:
            # Decoupling = sg_max much higher than typical AND much
            # higher than current median.
            if (last_max >= 3.0 * last_med
                    and last_max >= 1.5 * median_coarse_max
                    and last_max - last_med >= 200):
                return ("%s max %.0f in this step is %.1fx the median "
                        "(%.0f) and %.1fx the typical coarse max "
                        "(%.0f) — at least one repeat showed motor "
                        "decoupling spike (no-load value)"
                        % (sg_label, last_max, last_max / last_med,
                           last_med, last_max / median_coarse_max,
                           median_coarse_max))
        else:
            # SG4: decoupling = sg_min much lower
            if (last_min > 0
                    and last_min <= 0.3 * last_med
